...
def map_coordinates_3d(feats: torch.Tensor, coordinates: torch.Tensor) -> torch.Tensor:
    ...
    out = (
        F.grid_sample(x, y, mode="bilinear", align_corners=False, padding_mode="border")
        .squeeze(dim=(3, 4))
        .permute(0, 2, 1)
    )
    return out
...
def soft_argmax_heatmap_batched(softmax_val, threshold=5):
    ...
    weighted_sum = torch.sum(
        coords[None, None, None, :, :, :] * valid * softmax_val[:, :, :, :, :, None],
        dim=(3, 4),
    )
...
class _JaxBackend(backend.Backend[torch.Tensor]):
    ...
    def transpose(self, x: torch.Tensor, op: abstract_ops.Transpose) -> torch.Tensor:
        return x.permute(op.perm)

    def broadcast(self, x: torch.Tensor, op: abstract_ops.Broadcast) -> torch.Tensor:
