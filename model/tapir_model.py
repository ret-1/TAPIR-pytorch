...
class Tapir(nn.Module):
    ...
    def forward(
        ...
        out = dict(
            occlusion=torch.mean(torch.stack(trajectories["occlusion"][p::p]), dim=0),
        ...
        if points_gt is not None:
            loss = sequence_loss(
                trajectories["tracks"],
                points_gt.permute(0, 2, 1, 3),
                visibs_gt.permute(0, 2, 1),
                visibs_gt.permute(0, 2, 1),
            )
        else:
            loss = None
            ...
            if not utils_tap.is_same_res(curr_resolution, resolution):
                ...
                if self.feature_extractor_chunk_size > 0:
                    ...
                    for start_idx in range(0, video_resize.shape[0], chunk_size):
                        ...
                        u3 = resnet_out["resnet_unit_3"].permute(
                            0, 2, 3, 1
                        )  # .detach()
                        latent_list.append(u3)
                        u1 = resnet_out["resnet_unit_1"].permute(
                            0, 2, 3, 1
                        )  # .detach()
                    ...
                    latent = resnet_out["resnet_unit_3"].permute(
                        0, 2, 3, 1
                    )  # .detach()
